...
export default function UserTableToolbar({
  ...
  const handleDelete = async () => {
    ...
    const deletePromises = selected.map(async (DealerID) => {
      const apiUrl = `https://vlmtrs.onrender.com/v1/dealer/delete/${DealerID}`;
      try {
        const response = await fetch(apiUrl, {
          method: 'DELETE',
        });
        if (!response.ok) {
          throw new Error(`Failed to delete dealer with ID: ${DealerID}`);
        }
        return response.json();
      } catch (error) {
        console.error(`Error deleting dealer with ID: ${DealerID}`, error);
        throw error;
      }
    });
    ...
    try {
      ...
      toast.error('Some dealer could not be deleted. Please try again.');
    }
  };
  ...
  const handleDownloadExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Dealer'); // Append the worksheet to the workbook

    // Format the date and time
    const dateTimeString = formatDateTime();
    const fileName = `Dealer_${dateTimeString}.xlsx`.replace(/ /g, '_'); // Replace spaces with underscores
